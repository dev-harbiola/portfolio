...
function App() {
  ...
  return (
      <Container className={`${darkTheme && 'dark'} dark:bg-gray-800 mont-montserrat`}>
        ...
        <div className={'fixed sm:top-[12rem] md:top-3 sm:right-2 md:right-[2rem] z-50'}>
          {darkTheme ? (
            <MdLightMode
              className="sm:text-[2rem] md:text-[2rem] dark:text-gray-100"
              onClick={() => toggleTheme()}
            />
          ) : (
            <MdDarkMode
              className="sm:text-[2rem] md:text-[2rem] text-gray-800 dark:text-gray-100"
              onClick={() => toggleTheme()}
            />
          )}
        </div>
